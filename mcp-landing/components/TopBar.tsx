...
export default function TopBar() {
  const { href, handleClick } = useWhatsAppCTA({
    baseMessage:
      'Hola Marcelo, estoy viendo la pagina de cocinas a medida y quiero pedir un presupuesto.',
    ctaLabel: 'Pedir tu precio',
    placement: 'topbar',
  })
    ...
    <header className="bg-brand-dark/95 backdrop-blur-md sticky top-0 z-40 border-b border-white/5">
      <nav className="max-w-6xl mx-auto flex items-center justify-between px-5 py-4">
        ...
        <a
          ...
          className="flex items-center gap-2 bg-brand-green text-white font-semibold text-sm py-2.5 px-5 rounded-full hover:bg-brand-green-hover hover:shadow-glow-green transition-all duration-200"
        >
